...
if (isset($_POST["add"])) {
    ...
    if ($check !== false) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            $product_qstr = "INSERT INTO kwerekwe_products (name, description, price, image) VALUES ('$name', '$description', '$price', '$image')";
            $query = mysqli_query($conn, $product_qstr) or die("Operation Failed");

            header("location:kwerekwe_dashboard.php?sms=Product Added Successfully");
        } else {
            header("location:kwerekwe_dashboard.php?sms=Sorry, there was an error uploading your file.");
        }
    } else {
...
<head>
    ...
    <title>kwerekwe Dashboard</title>
...
<main>
    ...
    <div class="content">
        <h1 class="text-end">Manage kwerekwe Products</h1>

        <!-- Add Product Form -->
        <form action="kwerekwe_dashboard.php" method="POST" enctype="multipart/form-data">
            <div class="mb-3">
        ...
        <?php 
            $query_str = "SELECT * FROM kwerekwe_products";
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_array($query)) { ?>
                    <tr>
                            ...
                            <div class="modal fade" id="editProductModal<?php echo $row["id"]; ?>" tabindex="-1" aria-labelledby="editModalLabel<?php echo $row["id"]; ?>" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="kedit_products.php" method="POST" enctype="multipart/form-data">
                        ...
                        <td>
                            <a href="kdelete_product.php?id=<?php echo $row["id"]; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this product?');">
                                Delete
                            </a>
                        </td>
                    </tr>
